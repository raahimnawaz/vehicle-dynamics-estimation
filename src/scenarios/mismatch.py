...
Methods compared:
  ...
  - **PINN**  (mu_theta(s), pretrained; uses the slip-aware model for prediction)

Metric: trajectory RMSE between each method's *predicted* velocity and the
clean ground truth, both rolled forward with the method's appropriate model.
"""
...
from src.ml.pinn import MuNet
from src.ml.train import FrictionNet
from src.physics.wheel import DEFAULTS, mu_pacejka, ramp_slip
...
EFFECTS = (
    ...
    Effect("brake",    "brake ramp τ (s)",   (0.01, 0.15, 0.3, 0.5, 0.8),         "s"),
)


def simulate_truth(v0: float, t: np.ndarray, *, effect: str, intensity: float,
                   s_schedule: Callable[[float], float] | None = None,
                   mu_max: float = 0.9, C: float = 20.0) -> tuple[np.ndarray, np.ndarray]:
    """Slip-aware forward roll with one unmodeled effect active.

    Returns `(v_truth_clean, s_traj)`. The simple-model fitters never see
    s_traj, only the noisy v_truth. The PINN does see s_traj (it's an input
    feature, just like in training).
    """
    ...
    g = DEFAULTS["g"]
    m = DEFAULTS["m"]
    k = 0.4   # lumped 0.5 * rho * Cd * A (matches src/physics/wheel.py)
    ...
    for i in range(1, len(t)):
        ...
        def rhs(vi: float, tt: float) -> float:
            si = s_schedule(tt)
            mu = mu_pacejka(si, mu_max, C)
            ramp = 1.0 - np.exp(-tt / tau)
...
# ---------------------------------------------------------------------------
# Estimators -- each takes (t, v_obs, s_traj) and returns (label, v_predicted)
# ---------------------------------------------------------------------------

def fit_batch(t, v_obs, s_traj):
    dt = float(t[1] - t[0])
    v0 = float(v_obs[0])
...
def fit_ekf(t, v_obs, s_traj):
    dt = float(t[1] - t[0])
    v0 = float(v_obs[0])
    ekf = VehicleEKF(mu_init=0.5, v_init=v0)
    ekf.Q[1, 1] = 1e-2  # match the real-data tuning so EKF gets a fair shot
    for vi in v_obs:
        if vi > 1.0:
            ekf.predict(dt)
            ekf.update(vi)
...
def fit_nn(t, v_obs, s_traj, weights_path: str = "models/friction_net.pth"):
    dt = float(t[1] - t[0])
    v0 = float(v_obs[0])
    window_size = 50
    ...
    net.eval()
    # The pretrained net expects a window starting after a brief warm-up.
...
def fit_pinn(t, v_obs, s_traj, weights_path: str = "models/pinn_mu.pth"):
    """PINN uses the slip-aware model with mu_theta(s) for forward prediction."""
    dt = float(t[1] - t[0])
    v0 = float(v_obs[0])
    net = MuNet()
    net.load_state_dict(torch.load(weights_path, map_location="cpu", weights_only=True))
    ...
    # Forward-roll the slip-aware model: dv/dt = -mu_theta(s(t)) * g - k * v^2 / m
    # k=0.4 in lumped units to match the slip-aware simulator's drag (no road
    # grade / wind / ramp in the prediction -- we want to see how far this
    # gets even given the "perfect" model in the absence of mismatch).
    g = DEFAULTS["g"]
    k_over_m = 0.4 / DEFAULTS["m"]
    ...
    mu_eff = float(np.average(mu_traj[s_traj > 1e-4]))   # representative scalar
    return ("PINN", v_pred, mu_eff)


METHODS = (fit_batch, fit_ekf, fit_nn, fit_pinn)
...
def run_sweep(*, dt: float = 0.01, t_final: float = 3.5, v0: float = 28.0,
              noise: float = 0.25, seed: int = 0) -> list[SweepCell]:
    ...
    for effect in EFFECTS:
        for intensity in effect.intensities:
            v_truth, s_traj = simulate_truth(v0, t, effect=effect.name, intensity=intensity)
            v_obs = v_truth + rng.normal(0.0, noise, size=len(t))
            for method in METHODS:
                label, v_pred, mu_est = method(t, v_obs, s_traj)
