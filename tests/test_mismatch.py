...
def test_truth_simulator_decreasing():
    ...
    v, s = simulate_truth(28.0, t, effect="grade", intensity=0.0)
...
def test_grade_adds_deceleration():
    t = np.arange(0, 3.0, 0.01)
    v_flat,  _ = simulate_truth(28.0, t, effect="grade", intensity=0.0)
    v_steep, _ = simulate_truth(28.0, t, effect="grade", intensity=0.10)
    assert v_steep[-1] < v_flat[-1] + 1e-6, "uphill should decelerate more"


def test_headwind_adds_drag():
    t = np.arange(0, 3.0, 0.01)
    v_calm, _ = simulate_truth(28.0, t, effect="headwind", intensity=0.0)
    v_wind, _ = simulate_truth(28.0, t, effect="headwind", intensity=10.0)
...
def test_sweep_qualitative_ranking():
    ...
    # NN is consistently mismatched.
    assert means["NN (FrictionNet)"] > 0.10
    # PINN beats EKF on average (slip-aware model > constant-mu model).
